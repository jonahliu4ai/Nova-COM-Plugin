// NovaCOMPlugin.cs
// 版本: 4.2 C#5 Compatible (Fixed out var)
...
namespace NovaCOMPlugin
{
    ...
    public class StabilityController
    {
        ...
        private bool CheckStable(float cur, float max, float avg)
        {
            ...
            if (_history.Count < StabilizeTime * 0.5f)
                return false;
            if (max > MaxDeviation)
                return false;
            if (avg > AvgDeviation)
    ...
    public abstract class DeviceBase
    {
        ...
        public string CheckStability()
        {
            ...
            StateCode code = Stability.Update(pvsv.PV, pvsv.SV);
            switch (code)
            {
    ...
    public static class DeviceHelper
    {
        ...
        public static string GetTempString(DeviceBase dev)
        {
            if (dev == null)
                return "ERR";
            dev.CheckStability();
            return dev.Stability.LastPV.ToString("F1") + "/" + dev.Stability.LastSV.ToString("F1");
        }
