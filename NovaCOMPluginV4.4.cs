// NovaCOMPlugin.cs
// 版本: 4.4 Continuous Control Fix
// 修复:
//   1. 串口读写前清空输入缓冲区，解决连续写入时的 Err:Wrt8/Err:Wrt10
//   2. WriteSV 成功后自动重置状态机，支持连续温度控制
//   3. WaitUntilStable 开始时强制重置状态机，防止旧状态干扰
//   4. AIBUS 写响应按 10 字节读取，避免残留数据污染下次操作
//   5. 增加 LastTargetSV 跟踪，CheckStability 检测到 SV 变化时自动重置
//   6. 所有串口操作增加适当延时，确保设备处理完毕
...
namespace NovaCOMPlugin
{
    ...
    public abstract class DeviceBase
    {
        ...
        // 最后设定的目标温度（Nova 可读取）
        public float LastTargetSV
        {
            get { return _lastTargetSV; }
        }
        ...
        public string CheckStability()
        {
            ...
            // 如果设备当前 SV 与上次设定的目标温度差异较大，
            // 说明可能外部修改了 SV 或设定尚未生效，自动重置状态机
            if (Math.Abs(pvsv.SV - _lastTargetSV) > 1.0f)
            {
                _stability.Reset();
                _lastTargetSV = pvsv.SV;
            }
    ...
    public class AIBUSDevice : DeviceBase
    {
        ...
        private string Read()
        {
            ...
            // 关键修复：写之前清空输入缓冲区，避免残留数据干扰
            port.DiscardInBuffer();
            // 写之前短暂延时，确保设备已处理完上一个指令
        ...
        private string WriteSV(float sv)
        {
            ...
            // 关键修复：AIBUS 写指令响应也是 10 字节，按 10 字节读取
            byte[] resp = new byte[10];
            int n = 0;
            int wait = 0;
            while (n < 10 && wait < 500)
            ...
            // 验证响应：至少 8 字节，且第 3 字节为 0x43（写命令码）
            if (n >= 8 && resp[2] == 0x43)
            {
                // 关键修复：写入成功后，记录目标温度并重置状态机
                // 这样新温度下会重新从 HEATING 开始判断
                _lastTargetSV = sv;
                ResetStability();
                return "OK";
            }
    ...
    public static class DeviceHelper
    {
        ...
        public static string WaitUntilStable(DeviceBase dev, int timeoutSec)
        {
            if (dev == null)
                return "ERR:Null";
            // 关键修复：开始时强制重置状态机，确保每次调用都从头等待
            // 防止上一次稳定后的 STABLE 状态直接返回假阳性
            dev.ResetStability();
